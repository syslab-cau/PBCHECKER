...
// Moco insert
#include <time.h>
unsigned long long fsck_time = 0;
unsigned long long fsck_count = 0;

unsigned long long run_fsck_time = 0;
unsigned long long run_fsck_count = 0;
...
int main (int argc, char *argv[])
{
	...
	struct timespec run_local_time[2];
...
restart:
	...
	printf("try_open_fs start\n");	
	retval = try_open_fs(ctx, flags, io_ptr, &fs);

	printf("try_open_fs fin\n");	
...
//Moco run fsck part
	...
	clock_gettime(CLOCK_MONOTONIC, &run_local_time[0]);

	run_result = e2fsck_run(ctx);


	clock_gettime(CLOCK_MONOTONIC, &run_local_time[1]);
...
skip_write:
	...
	calclock(local_time,&fsck_time,&fsck_count);

	calclock(run_local_time,&run_fsck_time,&run_fsck_count);
	printTime_fsck();	
	return exit_value;
